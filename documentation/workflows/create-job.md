...
1. Obtain a human or delegated caller token carrying `synthetic-data:jobs` and authorization for action `create` on `synthetic-data/jobs`.
...
5. Persist the returned `job_id`, `request_digest`, state, and quota reservation for polling and audit.
